...
#Purpose: First change to OG script. Background, scoring monitor, pause button etc.
#Date: 31 Dec 2020
...
##        self.window.bind("<o>", self.unpause)        
    ...
    def place_apple(self):
        ...
        #Higher pay off thingy
        #Steps done: Random filter to get it, Random time element, 
        #Steps not to do: if not maybe "invisible display" that doesnt make a diff through a counter thing
        #Steps to do: Then ofc higher reward
        if round(time.time()) % 2 == 1:
            self.apple_cell2 = random.choice(list(unoccupied_cels))
            row_h = int(size_of_board / rows)
            col_w = int(size_of_board / cols)
            x1 = self.apple_cell2[0] * row_h
            y1 = self.apple_cell2[1] * col_w
            x2 = x1 + row_h
            y2 = y1 + col_w
            self.apple_obj2 = self.canvas.create_rectangle(
                x1, y1, x2, y2, fill=BLUE_COLOR_LIGHT, outline=BLUE_COLOR,
            )
        else:
            self.apple_cell2 = []
...
##                or head[1] < 0
            ...
            self.crashed = True                           
        elif self.apple_cell2 == head:
            # Got the apple2
            self.old_apple_cell = self.apple_cell2
            self.canvas.delete(self.apple_obj)
            self.canvas.delete(self.apple_obj2)
            app = True
            print(app)
            self.place_apple()
            self.display_snake()
