...
#######Code to get data and add indicators (MA, Candle types, Pivot)
#######Also a function to make candlestick charts

#Helpful bits that don't fit anywhere
#Install pandas into the computer (command prompt)
#py      -m pip install pandas
#To above to upgrade pip do --upgrade pip

#df.dropna()

#Print specific values
#print(df.groupby('Day').count())
#print(df.loc[df['<DATE>'] == '2020.07.28'])
#df[(df.Triple == 'Morning Star')]

#To get data types
#print(df.dtypes)

#Only get stuff with '1' in it
#df[df['Single'].str.contains("1") == True]

#Percentiles from numpy
#np.percentile(dfhAUD['Body'], 90)
...
#print(df[(df['<DATE>'] > pd.Timestamp(date(2019,3,1))) & (df['<DATE>'] < pd.Timestamp(date(2019,3,10)))])
...
def Run(df,name,ch):
    ...
    df.loc[(df['MA_200'] < 0.998*df['<CLOSE>']), '200_MA_Comp'] = 'Close Higher'
    df.loc[(df['MA_200'] > 1.002*df['<CLOSE>']), '200_MA_Comp'] = '200 Higher'
    df.loc[(df['MA_200'] <= 1.002*df['<CLOSE>']) & (df['MA_200'] >= 0.998*df['<CLOSE>']), '200_MA_Comp'] = 'Same'
    ...
    ##################CLOSE CHECK (RANGING DAYS)########################
    
    #A spell of x (3, 5, 10, 20) days where there was a move made but then the close is same as before
    #3 days (within week)
    df['Close_3'] = '0- Different'

    df.loc[(df['<CLOSE>'].shift(3) <= 1.002*df['<CLOSE>']) & (df['<CLOSE>'].shift(3) >= 0.998*df['<CLOSE>'])
           & (df['<CLOSE>'].shift(2) < df['<CLOSE>']) & (df['<CLOSE>'].shift(1) < df['<CLOSE>']), 'Close_3'] = '2- Same after bear3'

    df.loc[(df['<CLOSE>'].shift(3) <= 1.002*df['<CLOSE>']) & (df['<CLOSE>'].shift(3) >= 0.998*df['<CLOSE>'])
           & (df['<CLOSE>'].shift(2) > df['<CLOSE>']) & (df['<CLOSE>'].shift(1) > df['<CLOSE>']), 'Close_3'] = '2- Same after bull3'

    df.loc[(df['<CLOSE>'].shift(3) <= 1.002*df['<CLOSE>']) & (df['<CLOSE>'].shift(3) >= 0.998*df['<CLOSE>'])
           & (df['Close_3'] == '0- Different'), 'Close_3'] = '2- Same after unsure3'

    #5 days (one week later)
    df['Close_5'] = '0- Different'

    df.loc[(df['<CLOSE>'].shift(5) <= 1.002*df['<CLOSE>']) & (df['<CLOSE>'].shift(5) >= 0.998*df['<CLOSE>'])
           & ((df['<CLOSE>'].shift(4) + df['<CLOSE>'].shift(3))/2 < df['<CLOSE>']) &
           ((df['<CLOSE>'].shift(2) + df['<CLOSE>'].shift(1))/2 < df['<CLOSE>']), 'Close_5'] = '2- Same after bear5'

    df.loc[(df['<CLOSE>'].shift(5) <= 1.002*df['<CLOSE>']) & (df['<CLOSE>'].shift(5) >= 0.998*df['<CLOSE>'])
           & ((df['<CLOSE>'].shift(4) + df['<CLOSE>'].shift(3))/2 > df['<CLOSE>']) &
           ((df['<CLOSE>'].shift(2) + df['<CLOSE>'].shift(1))/2 > df['<CLOSE>']), 'Close_5'] = '2- Same after bull5'

    df.loc[(df['<CLOSE>'].shift(5) <= 1.002*df['<CLOSE>']) & (df['<CLOSE>'].shift(5) >= 0.998*df['<CLOSE>'])
           & (df['Close_5'] == '0- Different'), 'Close_5'] = '2- Same after unsure5'

    #10 days (two weeks later)
    df['Close_10'] = '0- Different'

    df.loc[(df['<CLOSE>'].shift(10) <= 1.002*df['<CLOSE>']) & (df['<CLOSE>'].shift(10) >= 0.998*df['<CLOSE>'])
           & ((df['<CLOSE>'].shift(8) + df['<CLOSE>'].shift(7) + df['<CLOSE>'].shift(6))/3 < df['<CLOSE>']) &
           ((df['<CLOSE>'].shift(2) + df['<CLOSE>'].shift(3) + df['<CLOSE>'].shift(4))/3 < df['<CLOSE>']), 'Close_10'] = '2- Same after bear10'

    df.loc[(df['<CLOSE>'].shift(10) <= 1.002*df['<CLOSE>']) & (df['<CLOSE>'].shift(10) >= 0.998*df['<CLOSE>'])
           & ((df['<CLOSE>'].shift(8) + df['<CLOSE>'].shift(7) + df['<CLOSE>'].shift(6))/3 > df['<CLOSE>']) &
           ((df['<CLOSE>'].shift(2) + df['<CLOSE>'].shift(3) + df['<CLOSE>'].shift(4))/3 > df['<CLOSE>']), 'Close_10'] = '2- Same after bull10'

    df.loc[(df['<CLOSE>'].shift(10) <= 1.002*df['<CLOSE>']) & (df['<CLOSE>'].shift(10) >= 0.998*df['<CLOSE>'])
           & (df['Close_10'] == '0- Different'), 'Close_10'] = '2- Same after unsure10'

    #20 days (two weeks later)
    df['Close_20'] = '0- Different'

    df.loc[(df['<CLOSE>'].shift(20) <= 1.002*df['<CLOSE>']) & (df['<CLOSE>'].shift(20) >= 0.998*df['<CLOSE>'])
           & ((df['<CLOSE>'].shift(18) + df['<CLOSE>'].shift(16) + df['<CLOSE>'].shift(14))/3 < df['<CLOSE>']) &
	   ((df['<CLOSE>'].shift(12) + df['<CLOSE>'].shift(10) + df['<CLOSE>'].shift(8))/3 < df['<CLOSE>']) &
           ((df['<CLOSE>'].shift(6) + df['<CLOSE>'].shift(4) + df['<CLOSE>'].shift(2))/3 < df['<CLOSE>']), 'Close_20'] = '2- Same after bear20'

    df.loc[(df['<CLOSE>'].shift(20) <= 1.002*df['<CLOSE>']) & (df['<CLOSE>'].shift(20) >= 0.998*df['<CLOSE>'])
           & ((df['<CLOSE>'].shift(18) + df['<CLOSE>'].shift(16) + df['<CLOSE>'].shift(14))/3 > df['<CLOSE>']) &
	   ((df['<CLOSE>'].shift(12) + df['<CLOSE>'].shift(10) + df['<CLOSE>'].shift(8))/3 > df['<CLOSE>']) &
           ((df['<CLOSE>'].shift(6) + df['<CLOSE>'].shift(4) + df['<CLOSE>'].shift(2))/3 > df['<CLOSE>']), 'Close_20'] = '2- Same after bull20'

    df.loc[(df['<CLOSE>'].shift(20) <= 1.002*df['<CLOSE>']) & (df['<CLOSE>'].shift(20) >= 0.998*df['<CLOSE>'])
           & (df['Close_20'] == '0- Different'), 'Close_20'] = '2- Same after unsure20'
    ...
    df['Close+1'] = df['<CLOSE>'].shift(-1) - df['<CLOSE>']
    df['Close+2'] = df['<CLOSE>'].shift(-2) - df['<CLOSE>']
    df['Close+3'] = df['<CLOSE>'].shift(-3) - df['<CLOSE>']
...
###EURUSD
##df1 = pd.read_csv (r'C:\Users\nitis\Documents\Forex\Data\EURUSD_Daily_Jan19-Sep20.csv',sep='\t')
##dfEUR = Run(df1,'EURUSD')
##
###GBPUSD
##df2 = pd.read_csv (r'C:\Users\nitis\Documents\Forex\Data\GBPUSD_Daily_Jan19-Sep20.csv',sep='\t')
##dfGBP = Run(df2,'GBPUSD')
##
###AUDUSD
##df3 = pd.read_csv (r'C:\Users\nitis\Documents\Forex\Data\AUDUSD_Daily_Jan19-Sep20.csv',sep='\t')
##dfAUD = Run(df3,'AUDUSD')
##
###EURGBP
##df4 = pd.read_csv (r'C:\Users\nitis\Documents\Forex\Data\EURGBP_Daily_Jan19-Sep20.csv',sep='\t')
##dfEURG = Run(df4,'EURGBP')
##
###ALL
##df = pd.concat([dfEUR,dfGBP,dfAUD,dfEURG], ignore_index=True)
###df = Run(dfAll)
...
#Checks possible for- Single, Double, Triple, MA_Cross, Pivot_Lev
#Currencies to choose: dfEUR, dfGBP, dfAUD, dfEURG, df(all currencies)
#Check(dfhEUR,['Single','MA_Cross'])
#Candle(dfhEUR,'EURUSD')

#Trying to chart only Bull Hammers
##df2 = df[df['Single'] == '1- Bull Hammer']
##d = go.Scatter(x = df2['Close+1'], y = df['Close+2'])
##figSignal = go.Figure(data=d)   
##figSignal.show()

#DONE
#Needs charting to test assumptions and be able to see key points:
#1. Response. Close of next day, next few days, weeks?: For now Close for next three days to check seperate
#2. Hammer upwick/tail; Body multiple: Approved. Added full bar
#3. Engulf just higher Body okay: Approved
#4. Tweezers' Low/High ranges: Added open/close condition
#5. Stars' and others' multiple ones: Approved
#6. Definition of Cross and Comp for MAs. How to find the sleeping ones? Like near Close already and then cross: Approved. Sleep is a lot of Sames
#Include pivots' crossing criteria etc. 
#Check for crossing conditions: Approved
#AND conditions for MA cross (maybe applies for the next 3-5 days or something), candle types, pivots. I need more data, this isnt enough
